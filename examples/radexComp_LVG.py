...
lognHgrid = arange(2, 8.01, 0.5)
logcolDengrid = arange(14, 24.01, 0.5)
...
for lognH in lognHgrid:
    ...
    for logcolDen in logcolDengrid:
        ...
        lines=cloud.lineLum('co', noClump=True, lumOnly=True, \
                                escapeProbGeom='LVG', abstol=1e-4)
        d=cloud.lineLum('co', noClump=True, 
                                escapeProbGeom='LVG', abstol=1e-4)
        t2 = datetime.now()
        timeDesp += t2 - t1
        ...
        tauList.append([d[k]['tau'] for k in d])
...
tauDiff = zeros((len(lognHgrid), len(logcolDengrid)))
for i, lognH in enumerate(lognHgrid):
    for j, logcolDen in enumerate(logcolDengrid):
